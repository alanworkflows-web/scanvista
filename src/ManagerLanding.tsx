...
export function ManagerLanding() {
  ...
  const handleDismiss = () => {
    setShowModal(false);
    setError("401 Unauthorized: Session Token Missing. Please authenticate to continue.");
  };
      ...
      <div className="md:w-1/2 p-8 md:p-16 flex flex-col justify-center bg-white">
        <div className="max-w-md w-full mx-auto">
          ...
          <form onSubmit={triggerGoogleLogin} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full bg-white border border-gray-300 text-gray-700 font-medium py-3 rounded-xl hover:bg-gray-50 transition-colors mt-6 flex justify-center items-center gap-3 shadow-sm"
            >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-1 text-center">Choose an account</h3>
              <p className="text-sm text-gray-500 mb-6 text-center">to continue to ScanVista</p>
              
              <div className="space-y-2">
                {[
                  { name: "Host Admin", email: "host@oceanhotel.com" },
                  { name: "Personal Account", email: "personal@gmail.com" }
                ].map((acc, i) => (
                  <button
                    key={i}
                    onClick={() => handleAccountSelect(acc.email)}
                    className="w-full flex items-center gap-4 p-3 hover:bg-gray-50 rounded-xl transition-colors border border-transparent hover:border-gray-200 text-left"
                  >
